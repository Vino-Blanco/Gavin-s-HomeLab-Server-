...
When we enabled VLAN-aware on vmbr1, OPNsense's NIC had no trunk configuration. The bridge started filtering tagged traffic, so OPNsense stopped receiving VLANs 10, 20, 30, and 40. The fix was adding `trunks=10;20;30;40` to OPNsense's net1 definition so the bridge knew to forward those tagged VLANs.

### PVID matters for untagged traffic
The switch management interface uses VLAN 1 (untagged). When we enabled VLAN-aware on vmbr1 with `bridge-vids 2-4094`, VLAN 1 was excluded, and untagged management traffic was dropped. Adding `bridge-vids 1-4094` and `bridge-pvid 1` fixed this by telling the bridge to accept and properly handle untagged VLAN 1 traffic.
...
The JGS516PE came with an old VLAN config that blocked traffic in unexpected ways. We had to factory reset it before our configuration would work. If you buy a used managed switch, reset it first.

### Don't remove yourself from the management VLAN
When configuring VLANs on the switch, we accidentally removed the management port from VLAN 1, locking ourselves out. The switch had to be factory reset again. Always ensure your management access remains intact before applying VLAN changes.
